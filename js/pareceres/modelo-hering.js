// Modelo "Hering": Auxiliar de Loja, Caixa, Coordenador(a) de Loja, Gerente
...
(function () {
  ...
  const GESTORES = ['Adriana de Jesus Barbosa', 'Anna Carolina Da Silva Dos Santos', 'Joelma de Souza Barbosa Cabral', 'Lucas Favareto', 'Marcia Maria De Souza Saraiva', 'Maria Elani Deodato De Souza', 'Maria Luiza Andrade Costa', 'Marisa Christina Ramos', 'Mateus Alves de Lacerda', 'Renan Alves de Oliveira', 'Vanessa Cavalcanti Silva', 'Odir Luiz Ferreira Garcez'];
  ...
  const LOJAS = ['Hering Américas Shop', 'Hering Catete', 'Hering Copacabana', 'Hering Icaraí', 'Hering Plaza', 'Hering Rio Sul'];
  ...
  const TREINAMENTO_HERING = {
    ...
    itens: [
      ...
      { key: 'obrigatoriedadeMensal', label: 'A obrigatoriedade de treinamentos mensais (presenciais e/ou em plataformas digitais como Academia Hering ou Twigo) foi explicada?' },
  ...
  const REMUNERACAO_VENDEDOR = {
    ...
    itens: [
      ...
      { key: 'pisoCategoria', label: 'O piso da categoria (salário mínimo garantido) foi informado?' },
      { key: 'metasSemanais', label: 'As metas semanais foram explicadas (ciclo domingo a sábado)?', hint: 'Nossas metas são semanais — começam no domingo e terminam no sábado. Isso significa que você tem aproximadamente 4 oportunidades por mês de bater suas metas e ganhar o máximo.' },
      { key: 'premiacoesIncentivo', label: 'As premiações e políticas de incentivo, após os 3 meses, foram detalhadas?' },
      { key: 'adiantamentoSalarial', label: 'Foi explicado o adiantamento salarial após 3 meses?', hint: 'Após o período de experiência, até o dia 20 de cada mês você vai receber um adiantamento do salário. É normal — é uma prática da empresa. O saldo restante cai no fechamento do mês.' },
      { key: 'valeTransporte', label: 'A política de Vale-Transporte foi apresentada (desconto de 6% conforme CLT)?', hint: 'Vale a pena saber: o desconto do VT é de 6% do seu salário, conforme a lei. Se você mora perto ou vai de bicicleta, pode optar por não pedir o benefício.' },
      { key: 'indicadoresDesempenho', label: 'Os principais indicadores de desempenho foram mencionados (Ticket Médio, Taxa de Conversão, Fluxo, PA, etc)?' },
  ...
  const ESCOPO_POR_GRUPO = {
    'Auxiliar de Loja': {
      ...
      itens: [
        ...
        { key: 'tarefasPraticas', label: 'Foram explicadas as tarefas práticas e rotinas operacionais do cargo, como abastecimento de PDV, organização de prateleiras, organização da loja, passadoria, recebimento e conferência de mercadorias?' },
    ...
    'Vendedor(a)': {
      ...
      itens: [
        ...
        { key: 'tarefasPraticas', label: 'Foram explicadas as tarefas práticas e rotinas operacionais do cargo, como organização da loja, reposição, ajuste de preço, organização de estoque, passadoria e apoio à loja?' },
        { key: 'padraoAtendimento360', label: 'O Padrão de Atendimento ao Cliente foi apresentado?', hint: 'Mencionar o modelo de Atendimento 360 da Hering: explicar que se trata de uma abordagem omnichannel, centrada no cliente, que integra todos os pontos de contato (loja física, digital e pós-venda), e que o colaborador será treinado nesse padrão.' },
  ...
  window.HUB_PARECER_MODELOS.hering = {
    id: 'hering', nome: 'Hering',
    gestores: GESTORES, recrutadores: C.RECRUTADORES,
    cargos: CARGOS,
    localizacao: { tipo: 'loja_simples', lojas: LOJAS, labelLocal: 'Para qual Loja Hering?' },
    grupos: GRUPOS,
    ...
    topicosPorGrupo: {
      'Auxiliar de Loja': [VD.BLOCO_JORNADA_VD, ESCOPO_POR_GRUPO['Auxiliar de Loja'], REMUNERACAO_PADRAO, TREINAMENTO_HERING, VD.BLOCO_ROTINA_VD, UNIFORME_HERING, C.BLOCO_PROXIMOS_PASSOS],
      'Vendedor(a)': [C.BLOCO_JORNADA_LOJA_B, ESCOPO_POR_GRUPO['Vendedor(a)'], REMUNERACAO_VENDEDOR, TREINAMENTO_HERING, VD.BLOCO_ROTINA_VD, UNIFORME_HERING, C.BLOCO_PROXIMOS_PASSOS],
      'Caixa': [C.BLOCO_JORNADA_LOJA_B, ESCOPO_POR_GRUPO['Caixa'], REMUNERACAO_PADRAO, TREINAMENTO_HERING, VD.BLOCO_ROTINA_VD, UNIFORME_HERING, C.BLOCO_PROXIMOS_PASSOS],
      [GRUPO_LIDERANCA]: [C.BLOCO_JORNADA_LOJA_B, ESCOPO_POR_GRUPO[GRUPO_LIDERANCA], REMUNERACAO_PADRAO, TREINAMENTO_HERING, VD.BLOCO_ROTINA_VD, UNIFORME_HERING, C.BLOCO_PROXIMOS_PASSOS]
    }
  };
})();
